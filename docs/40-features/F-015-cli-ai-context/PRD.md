...
wiring. This feature lets `tenancy init` optionally drop a stack-specific `TENANCY.md` and register a
...
- Opt-in only: an interactive yes/no prompt during `tenancy init --apply`, or the `--ai-context` flag
