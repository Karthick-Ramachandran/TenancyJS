...
<Callout type="info">
  **Enforcement tiers (ADR-0033).** *Database-enforced* means the database itself refuses cross-tenant
  rows (database-per-tenant, or Postgres row-level under forced RLS). *Facade-enforced* means the
  adapter facade is the only guard - raw SQL, joins, and nested writes are rejected fail-closed to stay
  safe.
</Callout>
...
<Callout type="warn">
  **Facade-only placements have no database backstop.** MySQL row-level is experimental and
  facade-only, MongoDB/Mongoose is facade-only, and Prisma row-level has no RLS backstop - in all of
  these the adapter facade is the entire guarantee. If you need a hard database-level guarantee on those
  stacks, prefer **database-per-tenant**. See the
