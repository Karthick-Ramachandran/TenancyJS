...
<Callout type="info">
  **How to use it:** open the setup-agent prompt for your stack below, copy it whole, and paste it into
  an assistant that can edit your repo (Claude Code, Cursor, …). It executes step by step and verifies
  each change. Then **run the two-tenant test it writes** - with fail-closed isolation, you verify, you
  don't trust.
</Callout>

## Full setup-agent prompts

These are complete, imperative, verify-after-each-step prompts - no placeholders to fill. Each drives an
AI agent through the entire wire-up for one stack and finishes by proving isolation.

<Cards>
  <Card
    title="NestJS + Prisma"
    href="/docs/stacks/nestjs-prisma#setup-agent-prompt"
    description="Row-level on PostgreSQL - the full setup-agent prompt plus a manual walkthrough."
  />
</Cards>

<Callout type="info">
  More stacks are being turned into full setup-agent prompts. Until yours has a dedicated page, the quick
...
Copy as-is (or adapt). Shorter than the full setup-agent prompts above - they point the assistant at the
