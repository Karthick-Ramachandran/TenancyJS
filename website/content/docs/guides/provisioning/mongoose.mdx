...
npx tenancyjs-cli tenant provision acme               # 2. ensure the tenant's indexes
npx tenancyjs-cli tenant migrate acme                 # 3. re-sync indexes / data migrations
...
appears on first write. Roll a change out to everyone with `npx tenancyjs-cli tenant migrate --all` - it reports
