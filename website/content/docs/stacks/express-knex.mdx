...
description: Wire fail-closed multi-tenancy into an Express 5 + Knex app with database-enforced Postgres RLS - a copy-pasteable setup-agent prompt, plus the manual walkthrough.
---

`tenancyjs-cli init` doesn't scaffold Express + Knex yet, so this stack is a manual wire-up - a few files. This
...
<Callout type="warn">
  ...
  Run both before you trust the result.
</Callout>
