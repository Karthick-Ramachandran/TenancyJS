...
<Callout type="warn">
  **TenancyJS is ESM-only; `nest new` scaffolds CommonJS.** The wiring is correct either way, but a
  CommonJS NestJS project will not start or run Jest against these packages until it is in ESM mode:
  `"type": "module"` in package.json, `module`/`moduleResolution` set to `"nodenext"` in tsconfig,
  `.js` extensions on relative imports, and tests on [Vitest](https://vitest.dev) or Jest ESM mode.
</Callout>
