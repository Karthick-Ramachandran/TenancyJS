...
description: Wire fail-closed multi-tenancy into an Express 5 + TypeORM app with forced PostgreSQL RLS - a copy-pasteable setup-agent prompt, plus the manual walkthrough.
---

`tenancyjs-cli init` doesn't scaffold Express + TypeORM yet, so this stack is a manual wire-up - a few files.
