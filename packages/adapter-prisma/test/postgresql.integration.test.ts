...
describePostgres("Prisma PostgreSQL row-level isolation", () => {
  ...
  it("isolates bulk create, update, returning, and delete operations", async () => {
    const prisma = client();
    await manager.runWithTenant({ id: "tenant-a" }, () =>
      prisma.post.createMany({
        data: [
          scopedPostData({ title: "A1" }),
          scopedPostData({ title: "A2" }),
        ],
      }),
    );
    await manager.runWithTenant({ id: "tenant-b" }, () =>
      prisma.post.create({ data: scopedPostData({ title: "B1" }) }),
    );

    await manager.runWithTenant({ id: "tenant-a" }, async () => {
      const created = await prisma.post.createManyAndReturn({
        data: [scopedPostData({ title: "A3" })],
      });
      ...
      expect(updated.every((post) => post.tenantId === "tenant-a")).toBe(true);

      await expect(prisma.post.deleteMany()).resolves.toEqual({ count: 3 });
    });

    await expect(base.post.findMany()).resolves.toEqual([
      expect.objectContaining({ tenantId: "tenant-b", title: "B1" }),
    ]);
  });
