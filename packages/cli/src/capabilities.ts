...
export function capabilityBanner(nodeVersion: string): string {
  ...
  return [
    ...
    "Supported stacks:",
    supported,
    "Isolation: single-database, shared-schema, row-level.",
    "  Enforced by forced Postgres RLS (Lucid/Knex) or query-scoping (Prisma).",
    "  Schema-per-tenant and database-per-tenant are in progress, not yet available.",
