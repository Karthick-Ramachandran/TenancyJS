  ...
  run(
    ...
    [
      ...
      [
        ...
        'if (tenantId !== "consumer" || outcome.status !== "resolved" || defineConfig({ strategy: "rowLevel" }).strategy !== "rowLevel" || prismaAdapter.name !== "prisma" || PRISMA_ADAPTER_CAPABILITIES.rawQueries !== "rejected" || typeof createKnexTenancy !== "function" || KNEX_ADAPTER_CAPABILITIES.rawQueries !== "rejected" || typeof createLucidTenancy !== "function" || LUCID_ADAPTER_CAPABILITIES.rawQueries !== "rejected" || typeof createExpressTenancyMiddleware !== "function" || typeof createNextTenancy !== "function" || createNextTenantHint(new Headers({ "x-tenant-id": "consumer" })) === null || redactText("postgresql://user:pass@localhost/db").includes("pass")) process.exit(1);',
      ].join("\n"),
    ],
    consumer,
  );
